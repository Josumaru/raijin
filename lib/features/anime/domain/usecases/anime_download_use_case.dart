...
class AnimeDownloadUseCase extends UseCase<void, DownloadParams> {
  @override
  Future call({required params}) async {
    final status = await Permission.storage.request();

    if (status.isGranted) {
      final baseStorage = Directory('/storage/emulated/0/Download/Raijin/');
      if (!await baseStorage.exists()) {
        await baseStorage.create(recursive: true);
      }
      ...
      final bool isQueue = existingTask!.any((task) {
        final status = task.filename == fileName && task.progress < 100;
        if (status == true) {
          return true;
        }
        return false;
      });
      final bool isDownloaded = existingTask.any(
          (task) => task.filename == '$fileName.mkv' && task.progress == 100);
